...
class DataProcessor:
    ...
    def get_train_examples(self):
        for id, value in df_train['triples'].iteritems():
          ...
          for triple in value:
            if triple['rank']>=2:
              pos_answer2.append(triple['c_et'])
              pos_rank2.append(triple)
            elif triple['rank']>=1:
                pos_answer.append(triple['c_et'])
...
#files directory
name = './data/3-all.json'
test_dir = './data/qblink-test-5-ordered-bulk.json'
train_dir = './data/qblink-train-5-ordered-bulk.json'
output_test_file = './data/test-new.txt'
...
for line in namef:
    ...
    question = jstr["question"]
    index = jstr["index"]
    name2index[question]=index
...
output_model_file = "./model/regressor1-ws.pth"
...
# Save a trained model
model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
output_model_file = "./model/regressor2-ws.pth"
#model.load_state_dict(torch.load(output_model_file))
#torch.save(model_to_save.state_dict(), output_model_file)
...
#Save a trained model
model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
output_model_file = "./model/regressor3-ws.pth"
#torch.save(model_to_save.state_dict(), output_model_file)
...
model.load_state_dict(torch.load("./model/regressor3-ws.pth"))
model.to(device)
