...
class CreateCer():
    ...
    def train_CR_model(self):
        if os.path.isfile(self.CRmodel_dir)==False:
            train_step(model_type="CR",model=self.CR_model,model_dir=self.CRmodel_dir,train_dir=self.train_dir,batch_size=self.train_batch_size,device=self.device)
    ...
    def test_CER_model(self):
        if os.path.isfile(self.final_dir)==False:
            self.CER_model.load_state_dict(torch.load(self.CERmodel_dir))
            self.CER_model.to(self.device)
